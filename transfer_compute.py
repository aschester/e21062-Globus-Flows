...
def run_flow(event_file=None):
    ...
    analysis_toplevel = '/global/cfs/cdirs/m4386/e21062_flows/analyzed'
    analysis_output_fname = f'run-{run_dir[3:]}-sorted.root'
    ...
    analysis_path  = os.path.join(analysis_toplevel, analysis_output_fname)
    ...
    pipeline_output = os.path.join(
        pipeline_output_toplevel, analysis_output_fname
    )
    ...
    logging.root.info(f'Analysis:  {analysis_path}')
    logging.root.info(f'Receiving: {analysis_path} --> {pipeline_output}')

    #########################################
    # Setup compute endpoints and functions #
    #########################################

    # Function for launching multiple instances of compute functions which
    # operate on .evt files as input (fitting, ROOT conversion):
    batch_function_id = 'e7e6549c-d3d8-47c2-86c8-261b5c4df474'

    # Multi-threaded fitting on the multi-node endpoint:
    compute_fit_ep_id = 'c1f86365-432f-47dc-92c3-6d909e3f128d'
    fit_function_id   = 'c9d94089-4666-4168-9989-b1acff850d0f'
    
    # The conversion/analysis is done on the single-node endpoint:
    compute_analysis_ep_id = '4dc55319-c672-42ac-b387-2e8dc6b3571a'
    convert_function_id    = 'd97891af-8c33-4d8e-8132-7436d79eff28'
    analysis_function_id   = 'f097b86b-6305-4946-beac-9e03abe5955d'
    ...
    flow_input = {
        ...
        'fit': {
            ...
            'function': batch_function_id,
            'kwargs': {
                'endpoint_id': compute_fit_ep_id,
                'function_id': fit_function_id,
                'ncpus': 64,
        ...
        'convert': {
            ...
            'kwargs': {
                ...
                'function_id': convert_function_id,
                'ncpus': 1,
                'input_path': fit_path
            },
        },
